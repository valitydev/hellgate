...
-spec handle_function(callback_name(), {tag()} | {tag(), callback()}, hg_service_wrapper:handler_opts()) ->
    term() | no_return().
    ...
    case hg_invoice:get(InvoiceRef) of
        {ok, InvoiceSt} ->
            case hg_invoice:get_payment(PaymentRef, InvoiceSt) of
                ...
                {error, notfound} ->
                    hg_service_wrapper:raise(#prxprv_PaymentNotFound{})
            end;
        {error, notfound} ->
            hg_service_wrapper:raise(#prxprv_PaymentNotFound{})
    end.
...
handle_callback_result({error, invalid_callback}) ->
    hg_service_wrapper:raise(#'InvalidRequest'{errors = [<<"Invalid callback">>]});
handle_callback_result({error, notfound}) ->
    hg_service_wrapper:raise(#'InvalidRequest'{errors = [<<"Not found">>]});
